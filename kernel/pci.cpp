...
namespace {
  ...
  /** @brief Writes info to devices[num_device] and increments num_device. */
   ...
   * If it's a PCI-PCI bridge, execute ScanBus for the secondary bus.
   */
   ...
   * If a valid function is found, execute ScanFunction.
   */
  ...
  /** @brief Scans each device of the specified bus number.
   * If a valid device is found, execute ScanDevice.
   */
  Error ScanBus(uint8_t bus) {
