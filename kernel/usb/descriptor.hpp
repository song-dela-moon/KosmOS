...
namespace usb {
  ...
  struct HIDDescriptor {
    ...
    struct ClassDescriptor {
      /** @brief Class-specific descriptor type value. */
      uint8_t descriptor_type;
      /** @brief Byte count of class-specific descriptor. */
      uint16_t descriptor_length;
    } __attribute__((packed));

    /** @brief Gets information about HID-specific descriptors.
     *
     * HID devices have one or more class-specific descriptors.
     * Their count is recorded in num_descriptors.
     * Report descriptor (type = 34) always exists for HID devices,
     * so num_descriptors is always at least 1.
     *
     * @param index  Descriptor index to retrieve. 0 <= index < num_descriptors.
     * @return Descriptor info for the given index. nullptr if index is out of range.
     */
