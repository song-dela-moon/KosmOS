 ...
 * @file usb/device.hpp
 *
 * Class representing USB device and related functionality.
 */
...
namespace usb {
  ...
  class Device {
   ...
   private:
    /** @brief Class drivers assigned to each endpoint.
     *
     * Index corresponds to endpoint number (0 - 15).
     * Index 0 is never used by any class driver, so it is always unused.
     */
    ...
    /** Map structure to identify the issuer of a request in OnControlCompleted.
     * Issuer is registered when ControlOut or ControlIn is issued.
     */
