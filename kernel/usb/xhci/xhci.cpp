...
namespace {
  ...
  /* For a root hub port, from reset until address assignment,
   * no other operations may be interleaved; only that port's processing must run.
   * kWaitingAddressed is the state waiting for the sequence from reset (kResettingPort)
   * through address assignment (kAddressingDevice).
   */

  std::array<volatile ConfigPhase, 256> port_config_phase{};  // index: port number

  /** Port number currently executing processing from kResettingPort to kAddressingDevice.
...
namespace usb::xhci {
  ...
  void Initialize() {
    // Search for xHC, prioritizing Intel devices
